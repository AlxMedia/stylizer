...
/* ------------------------------------ */
if ( ! function_exists( 'stylizer_hex2rgb' ) ) {

	function stylizer_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'stylizer_enqueue_google_fonts' ) ) {

	function stylizer_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'stylizer_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'stylizer_dynamic_css' ) ) {

	function stylizer_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = stylizer_hex2rgb($color_1);
...
#sidebar-top:before { border-color: '.esc_attr( get_theme_mod('color-bar-top-left') ).' transparent; }
			...
			wp_add_inline_style( 'stylizer-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'stylizer_dynamic_css' );
